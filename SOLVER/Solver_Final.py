...
def calculator(option, history):
	...
	def vector_calculator():
		...
		def vector_operations(vector1, vector2, operation=None, vector3=None):
			if operation == "1":
				...
				dot = input(
					"Select vectors for dot product (Enter 1,2 or 3 with a space): "
				).split()
				...
				cross = input(
					"Select vectors for cross product (Enter 1,2 or 3 with a space): "
				).split()
		...
		def menu():
			...
			print(
				"Enter '1' for vector addition, subtraction, dot product, cross product"
			)
			print(
				"Enter '2' to calculate vector triple product and scalar triple product and all the above"
			)
			print("Enter 'q' to quit")
			print()
	...
	def graphing_calculator():
		...
		def polynomial():
			...
			for i in range(int(degree), -1, -1):
				coefficient = parser_value(
					input("Enter the coefficient of x^{}: ".format(i))
				)
		...
		def trigonometry():
			function_type = input(
				"Enter 's' for sine, 'c' for cosine, 't' for tangent, 'cot' for cotangent, 'sec' for secant, or 'cosec' for cosecant: "
			)
			...
			def trigonometric_function(x):
				if function_type == "s":
					return (
						amplitude * np.sin(frequency * x + phase_shift) + vertical_shift
					)
				elif function_type == "c":
					return (
						amplitude * np.cos(frequency * x + phase_shift) + vertical_shift
					)
				elif function_type == "t":
					return (
						amplitude * np.tan(frequency * x + phase_shift) + vertical_shift
					)
				elif function_type == "cot":
					return (
						amplitude * np.divide(1, np.tan(frequency * x + phase_shift))
						+ vertical_shift
					)
				elif function_type == "sec":
					return (
						amplitude * np.divide(1, np.cos(frequency * x + phase_shift))
						+ vertical_shift
					)
				elif function_type == "cosec":
					return (
						amplitude * np.divide(1, np.sin(frequency * x + phase_shift))
						+ vertical_shift
					)
			...
			plot_function(
				trigonometric_function, start, end, num_points, "Trigonometric"
			)
		...
		def exponential():
			...
			base = parser_value(
				input("Enter the base of the exponential function (base): ")
			)
			coefficient = parser_value(input("Enter the coefficient (a): "))
			coeff = parser_value(input("Enter the multiplier coefficient (b): "))
	...
	def matrix_calculator():
		...
		def menu():
			...
			print(
				"Enter '1' for matrix addition, subtraction, multiplication and all of the below functions"
			)
			print(
				"Enter '2' for matrix transpose, inverse, determninat and eigenvalues and eigenvectors"
			)
			print("Enter 'q' to quit")
			print()
		...
		while True:
			...
			if operation == "q":
				...
				matrix1 = input(
					"Enter the first matrix (rows separated by ';', columns separated by ','): "
				)
				matrix2 = input(
					"Enter the second matrix (rows separated by ';', columns separated by ','): "
				)
				matrix1 = [
					list(map(parser_value, row.split(",")))
				...
				matrix1 = input(
					"Enter the matrix (rows separated by ';', columns separated by ','): "
				)
				matrix1 = [
					list(map(parser_value, row.split(",")))
					for row in matrix1.split(";")
	...
	def Ordinary_DE():
		...
		def differential_equation_generator(order):
			...
			for i in range(order, -1, -1):
				coefficient = function_parser(
					input("Enter the coefficient of order {}: ".format(i))
				)
		...
		def solution_DE(y, constant, order):
			...
			for i in range(order, -1, -1):
				if i == 1:
					...
					deqn = deqn.replace(
						"Derivative(f(x), (x, {}))".format(i), "f" + "'" * i + "(x)"
					)
