...
import os, math, json, time, datetime as datetime
...
def soln_parser(s):
	s=s.replace('\/`','√').replace('*',' × ')
	s=s.replace('e^', '𝓮^').replace('x','𝓍').replace('y','𝓎').replace('z','𝔃').replace('f','𝓯').replace('g','𝓰').replace('j','𝓳')
...
def main():
	...
	while True:
		...
		if option == 'q':  # EXIT PROCEDURE
			...
			history = calculator(option,history)
